...
- [ ] `go build ./...` exits 0
- [ ] `go test ./internal/project/...` exits 0
- [ ] `gofmt -l .` produces no output
- [ ] `go vet ./...` exits 0
- [ ] Coverage on `internal/project/...` ≥ 80%
- [ ] All 5 algorithm cases have at least one passing scenario test
- [ ] URL parsing covers ≥ 4 URL shapes (SSH, HTTPS+`.git`, HTTPS no-`.git`, malformed)
- [ ] All 6 locked decisions are visibly honored in the implementation
- [ ] No import of any `internal/*` package other than stdlib and `os/exec`
- [ ] `ErrAmbiguousProject` is a package-level `errors.New` sentinel, not a constructed error type
